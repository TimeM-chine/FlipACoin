...
local Sword = {}
Sword.__index = Sword
setmetatable(Sword, BaseWeapon)

Sword.CanHit = true

function Sword.new(systemIns, weaponInstance, ownerCharacter)
	local self = setmetatable(BaseWeapon.new(systemIns, weaponInstance, ownerCharacter), Sword)
...
function Sword:onEquippedChanged()
	if not IsServer then
		...
		if self.equipped then
			...
			animateScript.run.RunAnim.AnimationId = animFolder:FindFirstChild("SwordRun").AnimationId
			animateScript.walk.WalkAnim.AnimationId = animFolder:FindFirstChild("SwordWalk").AnimationId
			animateScript.idle.Animation1.AnimationId = animFolder:FindFirstChild("SwordIdle").AnimationId
			animateScript.idle.Animation2.AnimationId = animFolder:FindFirstChild("SwordIdle").AnimationId
			...
			local sword: Model = character:WaitForChild("SwordModel", 10)

			if sword then
				setupHitBoxPart(sword:WaitForChild("dmgPart"))
			else
				warn("no sword")
...
function Sword:onActivatedChanged()
...
function Sword:simulateFire(_firingPlayer, fireInfo, ownerCharacter)
	...
	local effectName = "Sword" .. curSlashNum
...
function Sword:stopAnimations()
	self.animTracks = {}
end

function Sword:cancelReload() end

function Sword:onDestroyed()
