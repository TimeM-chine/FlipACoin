...
function BaseWeapon.new(weaponsSystem, instance)
	...
	self.canReload = false -- sword can not reload
...
function BaseWeapon:setEquipped(equipped)
	...
	if (not IsServer) and self.player == Players.LocalPlayer then
		local character = self.player.Character or self.player.character
		local animateScript = character and character:FindFirstChild("Animate")
		if animateScript then
			local toolNoneAnim = animateScript:FindFirstChild("toolnone")
				and animateScript.toolnone:FindFirstChild("ToolNoneAnim")
			local runAnim = animateScript:FindFirstChild("run") and animateScript.run:FindFirstChild("RunAnim")
			local walkAnim = animateScript:FindFirstChild("walk") and animateScript.walk:FindFirstChild("WalkAnim")
			local idleAnim1 = animateScript:FindFirstChild("idle") and animateScript.idle:FindFirstChild("Animation1")
			local idleAnim2 = animateScript:FindFirstChild("idle") and animateScript.idle:FindFirstChild("Animation2")

			if self.equipped then
				if not self._savedAnimateIds then
					self._savedAnimateIds = {
						toolNone = toolNoneAnim and toolNoneAnim.AnimationId or nil,
						run = runAnim and runAnim.AnimationId or nil,
						walk = walkAnim and walkAnim.AnimationId or nil,
						idle1 = idleAnim1 and idleAnim1.AnimationId or nil,
						idle2 = idleAnim2 and idleAnim2.AnimationId or nil,
					}
				end

				local AnimatePresets = require(ReplicatedStorage.Systems.AnimateSystem.Presets)
				local weaponType = self.configValues.weaponType
				local moveAnim = weaponType and AnimatePresets.Animations.player[weaponType .. "Move"] or nil
				local idleAnim = weaponType and AnimatePresets.Animations.player[weaponType .. "Idle"] or nil

				if toolNoneAnim then
					toolNoneAnim.AnimationId = AnimatePresets.EmptyAnim
				end
				if runAnim and moveAnim then
					runAnim.AnimationId = moveAnim.id
				end
				if walkAnim and moveAnim then
					walkAnim.AnimationId = moveAnim.id
				end
				if idleAnim1 and idleAnim then
					idleAnim1.AnimationId = idleAnim.id
				end
				if idleAnim2 and idleAnim then
					idleAnim2.AnimationId = idleAnim.id
				end
			else
				local savedAnimateIds = self._savedAnimateIds
				if savedAnimateIds then
					if toolNoneAnim and savedAnimateIds.toolNone then
						toolNoneAnim.AnimationId = savedAnimateIds.toolNone
					end
					if runAnim and savedAnimateIds.run then
						runAnim.AnimationId = savedAnimateIds.run
					end
					if walkAnim and savedAnimateIds.walk then
						walkAnim.AnimationId = savedAnimateIds.walk
					end
					if idleAnim1 and savedAnimateIds.idle1 then
						idleAnim1.AnimationId = savedAnimateIds.idle1
					end
					if idleAnim2 and savedAnimateIds.idle2 then
						idleAnim2.AnimationId = savedAnimateIds.idle2
					end
				end
			end
...
function BaseWeapon:setActivated(activated, fromNetwork)
	...
	if IsServer and not fromNetwork then
		SystemMgr.systems.WeaponSystem.AllClients:WeaponActivated({
			player = self.player,
...
function BaseWeapon:onFired(firingPlayer, fireInfo, fromNetwork)
	if not IsServer then
		...
		if
			not firingPlayer
			or not firingPlayer.character
			or not firingPlayer.character.Humanoid
			or not firingPlayer.character:FindFirstChild("HumanoidRootPart")
		then
			return
		end
		...
		SystemMgr.systems.WeaponSystem.AllClients:WeaponFired({
			player = firingPlayer,
			instance = self.instance,
			fireInfo = fireInfo,
		})
...
function BaseWeapon:reload(player, fromNetwork)
	if
		...
		or self:getAmmoInWeapon() == self:getConfigValue("AmmoCapacity", 30)
	then
		return false
	end
	...
	if IsServer then
		...
		SystemMgr.systems.WeaponSystem.AllClients:WeaponReloaded({
			player = player,
			instance = self.instance,
		})
		...
		SystemMgr.systems.WeaponSystem.Server:WeaponReloadRequest({
			instance = self.instance,
		})
...
function BaseWeapon:onReloaded(player, fromNetwork)
	...
	local leftAmmo = self.instance:GetAttribute("Ammo")
	if leftAmmo <= 0 then
	...
	self.connections.reload = RunService.Heartbeat:Connect(function()
		...
		if self.ammoInWeaponValue then
			local ammoCapacity = self:getConfigValue("AmmoCapacity", 30)
			local newAmmo = math.min(self.ammoInWeaponValue.Value + leftAmmo, ammoCapacity)
			self.instance:SetAttribute("Ammo", leftAmmo - (newAmmo - self.ammoInWeaponValue.Value))
			self.ammoInWeaponValue.Value = newAmmo
		end
...
function BaseWeapon:cancelReload(player, fromNetwork)
	...
	if not IsServer and not fromNetwork and player == Players.LocalPlayer then
		SystemMgr.systems.WeaponSystem.Server:WeaponReloadCanceled({
			instance = self.instance,
		})
		-- self.weaponsSystem.getRemoteEvent("WeaponReloadCanceled"):FireServer(self.instance)
	elseif IsServer and fromNetwork then
		SystemMgr.systems.WeaponSystem.AllClients:WeaponReloadCanceled({
			player = player,
			instance = self.instance,
		})
	end
