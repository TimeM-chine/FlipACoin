...
do
	...
	function LinearSpring.new(dampingRatio, frequency, goal)
		assert(dampingRatio * frequency >= 0, "No steady state solution for the given damping ratio & frequency")
...
do
	function SpringService:Target(object, dampingRatio, frequency, properties)
		...
		for name, goal in pairs(properties) do
			...
			local meta = assert(springMetadata[typeof(goal)], "Unsupported type: " .. typeof(goal))
