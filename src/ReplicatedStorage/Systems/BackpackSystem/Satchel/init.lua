...
local Icon: any = require(Replicated.Packages.topbarplus)
...
local function MakeSlot(parent: Instance, initIndex: number?): GuiObject
	...
	slot.Frame = nil :: any
	slot.ItemCount = 1 :: number -- 物品数量，默认为1
	...
	local CountLabel: TextLabel = nil -- 数量显示标签
	...
	function slot:Fill(tool: Tool, count: number?): ...any
		...
		local function assignToolData(): ()
			...
			if CountLabel then
				if self.ItemCount > 1 then
					CountLabel.Text = `x{self.ItemCount}`
		...
		-- 监听工具属性变化（包括数量）
		local attrConn = tool:GetAttributeChangedSignal("itemCount"):Connect(function()
			local newCount = tool:GetAttribute("itemCount") or 1
	...
	-- 设置物品数量
	function slot:SetCount(count: number): ...any
		self.ItemCount = math.max(1, count)
		if CountLabel then
			if self.ItemCount > 1 then
				CountLabel.Text = `x{self.ItemCount}`
				CountLabel.Visible = true
	...
	CountLabel.TextStrokeTransparency = TEXT_STROKE_TRANSPARENCY
	CountLabel.TextStrokeColor3 = TEXT_STROKE_COLOR
	CountLabel.FontFace = Font.new(FONT_FAMILY.Family, Enum.FontWeight.Bold, Enum.FontStyle.Normal)
	CountLabel.TextSize = FONT_SIZE - 2
...
local function OnChildAdded(child: Instance): () -- To Character or Backpack
	...
	if slot then
		local itemCount = tool:GetAttribute("itemCount") or 1
		slot:SetCount(itemCount)
